@foreach ($allowanceCharges as $key => $allowanceCharge)
    <cac:AllowanceCharge>
        ...
        <cbc:Amount currencyID="{{$currency->CurrencyISO}}">{{number_format($allowanceCharge->amount, 2, '.', '')}}</cbc:Amount>
        @if ($allowanceCharge->base_amount)
            <cbc:BaseAmount currencyID="{{$currency->CurrencyISO}}">{{number_format($allowanceCharge->base_amount, 2, '.', '')}}</cbc:BaseAmount>
        @endif
    </cac:AllowanceCharge>
@endforeach
