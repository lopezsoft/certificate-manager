...
- **Bug semántico detectado por el usuario:** `InternalState::EXPIRED->toRequestStatus()` mapeaba a `CertificateRequestStatusEnum::EXPIRED` — el mismo valor que ya usa `MarkExpiredCertificatesJob` para "certificado emitido cuya vigencia venció" (`expiration_date`). Mezclar ambos significados bajo el mismo `request_status` visible habría sido confuso/contraproducente (una solicitud que nunca se emitió vs. un certificado que sí se emitió y venció son cosas completamente distintas). **Fix:** el mapeo ahora usa `CertificateRequestStatusEnum::CANCELLED` (ya existía en el enum, nunca se usaba en ningún lado — sin colisión). Actualizado también `ViafirmaRequestStateChangedListener::syncExpiredStatus()` para usar el mapper central en vez de un literal duplicado, y su comentario en `change_histories` ahora usa el mensaje dinámico real (antes tenía hardcodeado "(72h)", desactualizado).

### Corregido — Viafirma: el cupo NUNCA se reintegraba al cancelar por vencimiento de KYC (bug crítico)

- **Bug real detectado en producción (solicitud 1223):** tras cancelar la solicitud por vencimiento del plazo de verificación KYC, `certificate_order_items` seguía mostrando el item en `status: USED`, con `updated_at` sin cambios desde su creación — el cupo nunca se reintegró, sin ningún error visible.
- **Causa raíz:** `QuotaService::releaseQuotaForRequest()` solo busca items PREPAID con `certificate_order_items.certificate_request_id IS NULL`. El item consumido para esta solicitud sigue vinculado a su `certificate_request_id` (no se desvincula automáticamente al cancelar, solo al *eliminar* la solicitud vía `DeleteCertificateRequestHandler`, que sí hace ese desvincule antes de liberar). Como esta solicitud se **cancela pero no se borra**, el desvincule nunca ocurría y `releaseQuotaForRequest()` no encontraba nada que liberar — retornaba `false` silenciosamente, y `CancelExpiredKycRequestUseCase` descartaba ese resultado sin loguearlo.
- **Fix:** nuevo método `QuotaService::releaseQuotaForCancelledRequest(int $certificateRequestId, int $companyId): bool` — desvincula el item de esa solicitud puntual (`certificate_request_id = null`) y libera el cupo, en una sola transacción. `CancelExpiredKycRequestUseCase` ahora usa este método y **loguea un warning si el cupo no se pudo liberar** (`viafirma.kyc_expire.quota_not_released`), en vez de descartar el resultado en silencio.
- Test de regresión nuevo (mockeado, sin BD): `registra_warning_si_el_cupo_no_se_pudo_liberar`.
